...
from .model.model import PartPModel
...
class PartPainting():
    def __init__(self, config):
        self.config = config
        self.model_name = 'PartPModel'
        self.Model = PartPModel(config, '').to(config.DEVICE)
        
        self.psnr = PSNR(255.0).to(config.DEVICE)

        self.train_dataset = Dataset(config, config.TRAIN_FLIST, config.TEST_MASK, augment=False, training=True)
        self.val_dataset = Dataset(config, config.VAL_FLIST, config.TEST_MASK, augment=False, training=False)
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for it in train_loader:
                ...
                outputs, d_loss, d_p_loss, g_loss, logs = self.Model.process(data, pdata, half_fmask, ite)
  
                self.Model.backward(d_loss, d_p_loss, g_loss)
    ...
    def test(self):
        ...
        # features_path = os.path.join(self.results_path, 'part-patch')
        
        create_dir(input_data)
        create_dir(gt_data)
        create_dir(masks)
        create_dir(state_1_results)
        # create_dir(features_path)
        ...
        for it in test_loader:
            ...
            data, pdata, pos, fmask_data, half_fmask, mask, z = self.cuda(*it)
            o, ys = self.Model(pdata, fmask_data)

            up = nn.Upsample(size=256)
            # features = []
            # for i in range(len(ys)):
                # yys = torch.cat([torch.mean(ys[i], 1, True)]*3, dim=1)
                # yys = up(yys)
                # features.append(self.postprocess(yys)[0])
            ...
            # for i in range(len(features)):
                # imsave(features[i], os.path.join(features_path, 'layer_' + i + name))
    ...
    def sample(self):
        ...
        o_1s, ys = self.Model(pdata, half_fmask, pos, z)
        ...
        #     yys = torch.cat([torch.mean(ys[i], 1, True)]*3, dim=1)
        #     yys = up(yys)
        #     features.append(self.postprocess(yys))
